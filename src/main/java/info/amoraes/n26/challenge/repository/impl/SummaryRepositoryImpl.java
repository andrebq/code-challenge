...
public class SummaryRepositoryImpl implements SummaryRepository {
  ...
  /**
   * update summary adds the given summary to the current entry set, it will trim items that are
  ...
  public void updateSummary(Long minTimestamp, Long timestamp, Summary newItem) {
    ...
    try {
      ...
      entries.put(
          timestamp,
          Optional.ofNullable(entries.get(timestamp))
              .map(a -> Summary.merge(a, newItem))
              .orElse(newItem));
    } finally {
      writeLock.unlock();
    }
