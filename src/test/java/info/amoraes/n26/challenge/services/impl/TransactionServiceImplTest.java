...
public class TransactionServiceImplTest {
  ...
  public void addTransaction() throws Exception {
    ...
    verify(summaryRepository)
        .updateSummary(eq(1L - 60 * 1000L), eq(1L), eq(Summary.builder().from(inputTransaction)));
  ...
  public void shouldPreventTooOldTransactions() throws Exception {
    transactionService.addTransaction(
        1L, Transaction.builder().amount(1D).timestamp(-60 * 1000L).build());
  }

  @Test
  public void getSummary() throws Exception {
    when(summaryRepository.mergeEntriesAfter(any())).thenReturn(Summary.builder().build());
