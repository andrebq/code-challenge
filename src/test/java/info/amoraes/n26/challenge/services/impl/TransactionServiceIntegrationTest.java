...
public class TransactionServiceIntegrationTest {
    ...
    public void handlesMultithreadAccess() throws Exception {
        ...
        Summary expectedResult = Executors.newWorkStealingPool().invokeAll(
                Arrays.asList(generateItems(transactionService, now),
                ...
                }).reduce(Summary::merge).orElseThrow(() -> new RuntimeException("empty summary"));
    ...
    private Callable<Summary> generateItems(TransactionService service, Long now) {
        return () -> {
            ...
            return added.stream().map(t -> Summary.builder().from(t))
                    .reduce(Summary::merge).orElse(Summary.builder().build());
        };
    }
}
