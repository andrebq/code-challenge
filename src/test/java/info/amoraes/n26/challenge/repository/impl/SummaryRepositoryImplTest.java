...
public class SummaryRepositoryImplTest {
    ...
    @InjectMocks
    private SummaryRepositoryImpl repository;

    @Test
    public void shouldUpdateEntriesAndGetCorrectValue() throws Exception {

        Summary s = Summary.builder().from(
                Transaction.builder().amount(1D).timestamp(1000L).build());
    ...
    public void shouldTrimOldValues() throws Exception {
        Summary reallyOld = Summary.builder().from(
                Transaction.builder().amount(1D).timestamp(1L).build());
        repository.updateSummary(0L, 1L, reallyOld);

        Summary notSoOld = Summary.builder().from(
                Transaction.builder().amount(1D).timestamp(2L).build());
